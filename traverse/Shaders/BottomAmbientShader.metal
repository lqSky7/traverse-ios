...
[[ stitchable ]] half4 bottomAmbientLighting(
    ...
    float verticalFade = 1.0 - smoothstep(0.0, 0.68, bottomDistance);
    float deepBloom = exp(-bottomDistance * 4.0);
    float upperMist = exp(-bottomDistance * 2.8) * 0.28;
    ...
    float wideSource = 0.76 + sin(uv.x * 5.4 - time * 0.72) * 0.14;
    wideSource += sin(uv.x * 12.0 + time * 0.42) * 0.07;
    ...
    float leftGlow = 1.0 - smoothstep(0.0, 0.92, distance(uv, leftCenter));
    float middleGlow = 1.0 - smoothstep(0.0, 1.10, distance(uv, middleCenter));
    float rightGlow = 1.0 - smoothstep(0.0, 0.88, distance(uv, rightCenter));
    float sourceGlow = leftGlow * 0.75 + middleGlow * 0.88 + rightGlow * 0.70;
    ...
    float driftingMist = saturate(0.78 + risingNoise);
    ...
    float revealHeight = mix(0.0, 0.88, revealProgress);
    ...
    float wisps = risingBands * exp(-bottomDistance * 3.0);
    ...
    float volume = saturate(
        (deepBloom * 0.58 + upperMist * 1.25) * sourceGlow * wideSource * driftingMist * breathing
            + wisps * sourceGlow * 0.22
            + lightPockets * exp(-bottomDistance * 2.8) * 0.32
            + verticalFade * 0.15
    ) * organicReveal;

    float motes = 0.0;
    float2 moteSpace = float2(uv.x * aspectRatio, uv.y);
    for (ushort index = 0; index < 22; index++) {
        float seed = float(index) + 1.0;
        ...
        float radius = mix(0.0003, 0.0010, bottomAmbientRandom(seed * 6.1));
        ...
        motes += bottomAmbientSoftCircle(moteSpace, moteCenter, radius, 0.006)
            * moteFade;
    }

    half volumeAlpha = half(saturate(volume * 0.64));
    half moteAlpha = half(saturate(motes * 0.36 * smoothstep(0.18, 0.72, revealProgress)));
