...
// NOTE: CompleteRevisionResponse / RevisionAttemptRequest / RevisionAttemptResponse /
// RevisionAttempt / MLPrediction used to live here. They backed the old
// "mark revision done" flow (POST /api/revisions/:id/attempt), which no longer
// exists — the backend returns 404 for that route, and the app no longer offers a
// manual complete action. Do not re-add them without re-adding the endpoint.
